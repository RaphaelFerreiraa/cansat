...
void setup()
{
  ...
  MQ2.setRegressionMethod(1); /* Inicia o MQ2 */
  MQ2.init(); 

  bmp.begin(); /* Inicia o BMP180 */

  float calcR0 = 0; /* Formula de calibração do MQ2 */
    for(int i = 1; i<=10; i ++)
    {
      MQ2.update(); 
...
{ /* Conteúdo a enviar */
  ...
  data.CO = MQ2.readSensor(); MQ2.setA(605.18); MQ2.setB(-3.937);
  data.Alcohol = MQ2.readSensor(); MQ2.setA(77.255); MQ2.setB(-3.18); 
  data.NH4 = MQ2.readSensor(); MQ2.setA(102.2 ); MQ2.setB(-2.473); 
  data.Acetona = MQ2.readSensor(); MQ2.setA(34.668); MQ2.setB(-3.369); 
  data.CO2 = MQ2.readSensor(); MQ2.setA(110.47); MQ2.setB(-2.862); 
  ...
  MQ2.update(); /* Atualiza os dados do MQ2 */
